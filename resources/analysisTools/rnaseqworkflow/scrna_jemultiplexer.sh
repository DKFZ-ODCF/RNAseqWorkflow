...
BARCODE_JE=${outputAnalysisBaseDirectory}/barcode_jemultiplexer.txt
...
if [[ ${singleCellSequencingSystem} == "wafergen" || ${singleCellSequencingSystem} == "fluidigm" ]]; then
    ${PYTHON_BINARY} ${TOOL_PARSE_BARCODE_FILE} ${BARCODE_FILE} > ${BARCODE_JE}
...
if [ ${runAdapterTrimming} == true ]; then
    JE_OUTDIR_RAW=${JE_OUTDIR}/raw
else
    JE_OUTDIR_RAW=${JE_OUTDIR}
fi

make_directory $JE_OUTDIR_RAW
echo_run "$JE_BINARY FASTQ_FILE1=${READ_LEFT_COMBINED} FASTQ_FILE2=${READ_RIGHT_COMBINED} OUTPUT_DIR=${JE_OUTDIR_RAW} $JE_PARAMS"
